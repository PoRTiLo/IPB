...
V~souèasné dobì jsou øazeny k~nejroz¹íøenìj¹ím psaným dorozumívacím prostøedkùm komunikaèní sítì pracující v~reálném èase. Samozøejmì je tento jev k~vidìní a¾ koncem minulého desetiletí. Psaná komunikace je ji¾ nìkolik století vyu¾ívána jako prostøedek k~dorozumívání lidí mezi sebou. Za toto období pro¹la výrazným pokrokovým vývojem, proto lze u~ní nalézt mnoho èasových mezníkù, které ji výraznì ovlivnily. Jako pøíklad jednoho z~prvních komunikaèních kanálù lze uvést posly, kteøí èasto nesly zprávy na vzdálenosti nìkolika kilometrù. 
...
Pøíchodem internetu nastal v~komunikaci zásadní zlom. Postupným roz¹íøením pokrytí a~dostupnosti této technologie zaèalo vznikat mnoho nových komunikaèních prostøedkù. Pøíkladem mohou být elektronické zprávy, RSS zprávy nebo komunikaèní sítì, které pracují v~reálném èase. Právì poslední zmínìná metoda za¾ívala v~moderní dobì velký rùst v~oblasti popularity, a» u¾ v~podobì ICQ protokolu nebo dnes velmi roz¹íøené sociální síti Facebook. Jedna z~hlavních výhod tìchto slu¾eb, napøíklad v~porovnání s~klasickou po¹tou, je jejich rychlost doruèení. Naproti bì¾né elektronické po¹tì jsou u¾ivatele informováni o~stavu pøíjemce zprávy. Real-time komunikaèní prostøedky nabízejí slu¾by, kterými je mo¾né zjistit, zda se pøíjemce zprávy nachází u~dorozumívacího zaøízení. Díky této schopnosti je u¾ivateli umo¾nìno zasílat zprávy a~obratem na nì oèekávat odpovìdi.
...
Pøedmìtem této bakaláøské práce je seznámení se s~problematikou komunikace probíhající pøes Jabber sí». Konkrétním cílem je vytvoøení jednoduchého Jabber klienta, který by byl schopen získávat statistická data. Nashromá¾dìná data jsou dále vyu¾ita pro pozdìj¹í analýzu a~grafickou reprezentaci informací z~nich získaných. Hlavním cílem této práce je získat neznámé informace z~komunikaèní sítì Jabber. 

Technická zpráva je tvoøena z~¹esti kapitol.
Kapitola \textbf{druhá} je tvoøena popisem protokolu XMPP, na kterém je postavena komunikaèní slu¾ba Jabber pracující v~reálném èase. Je zde popsána architektura sítì a~základní stavební kameny XMPP protokolu, které jsou vyu¾ívány sítí Jabber jako nástroj k~zprostøedkování jednotlivých slu¾eb. V~závìru tohoto oddílu je vìnována èást vybraným standardùm, které roz¹iøují základní XMPP protokoly. Jsou zde uvedena pouze ta roz¹íøení, která byla po konzultaci s~vedoucím práce, oznaèena za relevantní k~této práci.

Obsah \textbf{tøetí} kapitoly je zamìøen na popis procesu data mining. Zabývá se zaèlenìním této metody do komplexnìj¹ího procesu, který je nazýván získávání znalostí z~databází. Dal¹í souèást této kapitoly se zabývá nezbytnými kroky transformace dat, která jsou získána z~Jabber komunikace. Pøi tomto procesu jsou data pøevedena do vhodné podoby pro proces dolovaní z~dat. Následuje èást, kterou jsou popsány vybrané metody dolování dat a~také je zde podrobnì rozebrán algoritmus \textit{k}-means. Tento algoritmus je vyu¾it pro samotné dolování z~dat, které je zprostøedkováno aplikací RapidMiner. Popis tohoto programu a~dal¹ích vybraných nástrojù uzavírá tøetí kapitolu.

Implementaèní èást této práce je popsána kapitolou \textbf{ètvrtou}. Je èlenìna na oddíly, které odpovídají vybraným èástem architektury této práce. Elementy architektury, které jsou popsány vlastní podkapitolou, jsou roz¹íøeny o~zjednodu¹ený návrh struktury v~podobì grafických schémat. Závìr této kapitoly se zamý¹lí nad roz¹íøeními robota a~nad dal¹ím pokraèováním této práce. 

Výsledky získané touto prací jsou prezentovány v~kapitole \textbf{páté}. Je zde ukázán model pou¾ívaný programem RapidMiner pro dobývání znalostí. Konkrétnì jde o~shlukování u¾ivatelù sítì Jabber do skupin podle èasu stráveném v~samotné síti a~jejich stavu. Výsledky jsou pøedstaveny pomocí rùzných grafických entit, a»  u¾ v~podobì diagramu nebo grafu, který je transformovaný do dvourozmìrného prostoru.

Nezanedbatelný informativní obsah tvoøí i~\textbf{pøílohy}. V~první øadì je to slovník zkratek, shrnující slovní spojení z~celého tohoto dokumentu. Následuje podrobnìj¹í popis základních entit stanzy a~pøehled prùbìhu roz¹íøení. V~dal¹í èásti je ukázán kompletní návrh databáze, který doplòuje základní popis ze ètvrté kapitoly. Pøehled pøíkazù, na které robot vytvoøený v~této práci reaguje, je uveden v~pøedposlední èásti pøíloh. Posledním oddílem jsou prezentovány výsledky získané procesem dolování z~dat a~následné transformace.
...
Dal¹í vymo¾enost JabberID oproti e-mailové adrese je jeho mo¾nost pou¾ívat prakticky libovolné národní znaky u~doménových jmen a~u¾ivatelských úètù \cite{Saint}. 
\begin{figure}[ht]
\begin{center}
\input{./pic/jabinfo}
...
\end{center}
\end{figure}
Vyu¾íváním kódování UNICODE, se XMPP stává plnì mezinárodní a~není jako jiné protokoly omezen rozsahem ASCII tabulky. Pøesto¾e je tato vymo¾enost k~dispozici, doposud není ¾ádným výrazným zpùsobem vyu¾ívána.
...
K~poslednímu roz¹íøení implementovanému v~této práci patøí \textit{User Tune} \cite{Xep0118}, které umo¾òuje u¾ivateli ¹íøit informace o~aktuálnì poslouchané hudbì. Nìkteré dne¹ní hudební pøehrávaèe doká¾í automaticky spolupracovat s~IM klientem a~pøedávat informace o~hudbì bez nutného lidského zásahu. Ve zprávì jsou tedy pøená¹eny informace o~skladbì, interpretovi, albu a~dal¹í informace, které mohou být získávány z~MP3 ID3v1 nebo novìj¹í ID3v2 tag.

Podpora vý¹e popsaných roz¹íøení v~aplikacích je pomìrnì malá.
...
Na zaèátku kapitoly je rozebrán pojem získávání znalostí databází, jeho¾ jednu podstatnou èást tvoøí samotný data mining. Dále je vysvìtlena základní terminologie, pro kterou bylo èerpáno z~\cite{IIS}. 
Cílem první podkapitoly je pøiblí¾ení zpùsobu, jakým byla data ulo¾ená v~databázi a~následnì pøipravena k~samotnému dolování z~dat.
Celý druhý oddíl je vìnován pøípravì dat pro samotné dolování z~dat. Je zde popsán proces transformace atributù na promìnné kvantitativní.
%Celá druhá podkapitola je vìnována vybraným metodám pro dolování dat a vlastnostem, které je od sebe navzájem odli¹ují. Jsou zde rozebrány \textit{asociaèní pravidla}, pro jejich¾ popis bylo èerpáno z \cite{Berka}. Pro ostatní metody, které jsou popsány dále, byla jako zdroj informací pou¾ita kniha \cite{Han}. 
Poté následuje tøetí podkapitola, která se podrobnìji zabývá jednou z~metod pro dolování dat a~to \textit{shlukováním}. Obsahem této èásti jsou ji¾ konkrétní algoritmy pro shlukování dat \cite{Rezankova,Bramer} a~také metoda \textit{k-Means} vyu¾ívaná v~praktické èásti této práce. Kapitolu uzavírá struèný pøehled vybraných programù pro data mining a~podrobnìj¹í seznámení s~nástrojem \textit{RapidMiner}, který je v~této práci vyu¾íván pro samotné dolování.


\subsection*{Terminologie}
Pojem data mining neboli èesky dolování dat se zaèal ve vìdeckých kruzích objevovat poèátkem 90. let 20. století. První zmínka pochází z~konferencí vìnovaných umìlé inteligenci (IJCAI'89\footnote{International Joint Conference on Artificial Intelligence} --- mezinárodní konference konaná v~Detroitu, AAAI'91\footnote{Association for the Advancement of Artificial Intelligence} a~AAAI'93 --- americké konference v~Kalifornii a~Washingtonu, D. C) \cite{Berka}. 
...
K~dal¹ímu popisu temporálních databází nyní budou charakterizovány tøi dùle¾ité pojmy, pomocí nich¾ jsou databáze dále dìleny. Prvním pojmem je \textit{granularita}, která udává nejmen¹í èasovou jednotku, kterou databáze rozli¹ují. Hodnoty, které mù¾e nabývat, jsou hodina, den, rok a~dal¹í. Velikost granularity ovlivòuje velikost objemu dat, který je pøímo úmìrný s~pøesností záznamù. Dal¹ím pojmem je \textit{èas platnosti}, která reprezentuje období, kdy je daný fakt v~modelovém svìtì pravdivý. Posledním termínem je \textit{èas transakce}, která definuje pøítomnost faktu v~databázi a~mo¾nost jej získat. Èas transakce a~èas platnosti jsou na sobì nezávislé a~definují dvì rozdílné èasové osy, kdy ka¾dá mù¾e disponovat s~jinou granularitou. Souhrnný název pro vý¹e uvedené tøi pojmy, který se pou¾ívá, je systém èasových razítek. Pøi pou¾ití tìchto systému lze následnì tvoøit dotazy zamìøené na rùzná èasová období jako je minulost, pøítomnost a~budoucnost. Tyto dotazy jsou velmi jednoduché a~to díky roz¹íøenému jazyku TSQL, který vychází z~klasické podoby dotazovacího jazyka SQL.
...
Postup jednotlivých krokù pøi transformaci èasových údajù z~tabulky \textit{presence} je zobrazen v~následujícím výètu. Tento zjednodu¹ený popis algoritmu vy¾aduje dva vstupní parametry. První je JabberID u¾ivatele, jeho¾ polo¾ky v~databázi mají být transformovány. Druhá nutná polo¾ka je datum, která bude slou¾it jako upøesòující vstupní interval.
...
%\section{Shlukování}\label{KapShluk}
%V této èásti podkapitole 
Následující èást se bude zabývat shlukováním, které je rozdìleno 
%je shlukování rozdìleno 
na nìkolik metod shlukové analýzy podle \cite{Han}. U~ka¾dé z~nich jsou popsány její základní vlastnosti a~uvedeny algoritmy relevantní k~práci. Poslední metoda \textit{metoda rozkladu} je rozebrána podrobnìji z~dùvodu jejího praktického vyu¾ití v~této práci.
...
RapidMiner je, tak jak je popsán na oficiálních stránkách produktu \cite{RapidMiner}, celosvìtovì nejpou¾ívanìj¹í open-source systém pro dolování dat. Je mo¾né jej pou¾ívat jako samotnou aplikaci nebo jej zaèlenit jako komponentu do vlastních výrobkù v~podobì knihovny pro jazyk Java. Pro zájemce je nabízen také ve verzích  pro firmy, které jsou rozdílné v~poplatcích, podpoøe pro zákazníka, záruce a~dal¹ích balíèkù slu¾eb zaji¹»ujících celkovou komplexnost a~spolehlivost produktu.

Jako ji¾ vìt¹ina podobných aplikací je RapidMiner v~souèasné dobì implementován v~jazyce Java, díky které nabízí flexibilní nejen grafické prostøedí. K~vybraným základním rysùm tohoto nástroje, tak jak jsou prezentovány firmou \textit{Rapid-i}, patøí: výkonné, pøesto intuitivní grafické u¾ivatelské rozhraní pro návrh procesù, jednoduché øe¹ení pro transformaci dat, kontrola výsledkù ji¾ pøi samotném návrhu a~dal¹í. Nástroj RapidMiner podporuje ¹irokou ¹kálu metod a~algoritmù pro data mining. Mnoho algoritmù je implementováno pøímo v~aplikaci, ale také je pou¾ito metod z~konkurenèního softwaru Weka. V~základní verzi urèené pro veøejnost je k~nalezení pøes 100 procesù k~modelování. Jsou zde zastoupeny jak metody klasifikaèní a~asociaèní, tak i~metody shlukovací, z~nich¾ lze jmenovat napøíklad DBSCAN, \textit{k}-medoids a~hlavnì \textit{k}-means. 
...
%První èást schématu, která je nazvaná XMPP sí», prezentuje obrázek \ref{picArchitektura}. Tento oddíl je obecnì popsána v druhé kapitole.
V~levé èásti obrázku \ref{picArchitektura} je blok XMPP sí», který prezentuje zdroj dat. Fungování XMPP sítì je detailnì popsáno ve druhé kapitole.
Vztah a~vzájemná komunikace s~robotem probíhá pomocí internetové sítì, kdy XMPP sí» souhrnnì reprezentuje jednotlivé prvky, jako jsou Jabber servery, u¾ivatele a~jejich klienty. Jak je patrné, výmìna informací mezi tìmito èástmi  probíhá obousmìrnì. Druhý element schématu \textit{robot}, je charakterizován v~podkapitole ní¾e, kde je podrobnì popsán návrh jeho struktury. Jsou zde zdùraznìny jeho základní vlastnosti a~schopnosti. Dal¹ím blokem je \textit{databáze}, která reprezentuje datové úlo¾i¹tì. Do nìho¾ jsou za pomocí jednosmìrného kanálu ukládána data, která jsou získávána z~toku informací proudícími mezi robotem a~XMPP sítí. Pøehled vybraných jednotlivých tabulek a~jejich atributù je mo¾né nalézt v~následující kapitole, která se zabývá návrhem databáze.

Dal¹ím oddílem, prezentovaným na obrázku \ref{picArchitektura} pod názvem \textit{data mining skript}, je skript, který provádí samotné dolování z~dat. Jako první jsou vybraná data z~databáze transformována do vhodného formátu pro dolování z~dat. Tato èást je podrobnì popsána ve tøetí kapitole, v~oddílu zabývajícím se transformací dat. Pøeformátovaná data jsou pøedána programu RapidMineru, kterým za pomocí algoritmu \textit{k}-means je provádìn data mining.

\begin{figure}[ht]
\begin{center}
\includegraphics[scale=.6]{./pic/drawing1.pdf}
\caption{Struktura architektury bakaláøské práce.}
 \label{picArchitektura}
\end{center}
\end{figure}

Poslední èástí je \textit{u¾ivatelské rozhraní}, které je implementováno pomocí webových slu¾eb. Jako pøíklad lze uvést slu¾bu, která zobrazuje status u¾ivatele na webové stránce. U¾ivateli pouze staèí vlastnit Jabber úèet a~mít pøidaného tohoto robota do seznamu kontaktù. Od ka¾dého u¾ivatele, je¾ má tohoto robota pøidaného do svého seznamù kontaktù, je sbírána ve¹kerá sí»ová aktivita. Mo¾nosti, které mu nabízí u¾ivatelské rozhraní v~podobì webové prezentace, tudí¾ zále¾í pouze na datech, které sám do sítì rozesílá. Jsou-li podporována v¹echna zde implementovaná roz¹íøení, která jsou uvedená v~kapitole druhé, je mo¾né vyu¾ít jejich slu¾eb prostøednictvím internetových stránek. Jako dal¹í pøíklad lze uvést zobrazení aktuálního umístìní u¾ivatele na mapách od firmy google, dle informací poskytnutých prostøednictvím dokumentu User Geoloc \cite{Xep0080}.
...
Data, která jsou sbírána robotem, jsou ukládána do objektovì-relaèní databáze PostgreSQL. PostgreSQL neboli také Postgres byl pou¾it ve verzi 8.4.7 a~je provozován na operaèním systému Ubuntu.
...
\section{Návrh robota}
V~této èásti bude popsána aplikace, která zaji¹»uje real-time komunikaci s~ostatními entitami  Jabber sítì. Konzolový robot, který je vyvíjen pro operaèní systém Linux, také vhodným zpùsobem pracuje s~databází, kam jsou ukládána jednotlivá získaná data. Jabber jádro sí»ové komunikace \cite{Rfc3920} je implementováno pomocí volnì dostupné knihovny \textit{gloox}. V~porovnání s~jinými knihovnami disponuje lep¹í podporou a~dokumentací. Gloox plnì implementuje standart XMPP Core \cite{Rfc3920} a~z~vìt¹í èásti i~standard XMPP IM \cite{Rfc3921}. Dodateènì je podporováno kolem tøiceti XEP standardù mezi nì¾ napøíklad patøí vcard-temp \cite{Xep0054}.% a dal¹í. 

Pøi implementaci robota byly vyu¾ity základní prvky objektovì-orientovaného programování. Ka¾dá tøída zde zastupuje jednoznaènì oddìlitelné elementy robota. A» u¾ se jedná o~blok komunikující s~databází, rùzná roz¹íøení nebo o~jádro robota. 
%V¹echny zde jmenované objekty hrají dùle¾itou roli jak v èásti dekompozice na men¹í ucelené celky tak i v celkové komplexnosti programu. 
Jako pøíklad lze uvést tøídy, které reprezentují jednotlivá pro tuto práci vhodná roz¹íøení, jejich¾ základ je v~podobì XEP dokumentù.

%\subsection*{Návrh robota}
Struktura návrhu tøíd robota, která je v~základní èásti prezentována obrázkem \ref{picRobot}, je velmi podobná struktuøe návrhu databáze, obrázek \ref{picDatabase}. Mnoho atributù z~robota a~z~databáze jsou ve vztahu jedna ku jedné. Samotná komunikace a~øízení mezi tìmito entitami je zprostøedkováno pomocí knihovny \textit{libpq}. 
%Libpq je programátorské rozhraní pro databázi PostgreSQL. Je to sada funkcí v jazyce C, které umo¾òují klientským programùm zadávat dotazy na server a následnì na nì obdr¾et odpovìdi. Pomocí této knihovny je øízen pøenos dat mezi robotem, který sbírá data, a databází, kam jsou ukládána. Navázání spojení a následné jednotlivé dotazy probíhají v blokujícím re¾imu. 
V~souboru \textit{connect.h} jsou, v~podobì pøeddefinovaných konstant, uvedeny v¹echny údaje nutné k~navázání spojení a» u¾ s~databází nebo s~komunikaèním Jabber serverem. 
%Tudí¾ se stává hlavní èástí urèenou k editaci pøipojení a Jabber úètu, na nìm¾ robot bì¾í. K dal¹ímu pouze textovému dokumentu patøí \textit{const.h},  který je zamìøen k definování jednotlivých pøíkazù a dotazù pro databázi. Jsou to pøíkazy pro tvorbu a aktualizaci tabulek, které jsou takté¾ definované jako konstanty. Identifikaèní názvy pro snadné odli¹ení jsou psány velkými písmeny a zaèínají textem \textit{DB\_}.

Komunikace s~databází se nachází v~samostatné tøídì, která zaji¹»uje provázanost mezi ní a~daty. Pomocí funkcí jsou data, získaná z~Jabber komunikace, pøená¹ena a~ukládána do jednotlivých tabulek databáze. Pøi zahájení èinnosti robota je zaji¹tìno navázání pøipojení k~databázi a~provedení nutných inicializaèních krokù. Jako je tvorba nových tabulek, pøi prvním zapnutí aplikace, nebo kontrola existence tìchto entit.

Pro roz¹íøení podle dokumentù XEP popsaných v~kapitole druhé, v~èásti zabývající se implementovanými roz¹íøeními, je základ nadtøída \textit{Extension}. Tato tøída obsahuje základní funkce a~parametry pro v¹echna roz¹íøení. 
%Jako pøíklad lze uvést u¾ivatelské jméno klienta, který dané roz¹íøení \uv{¹íøí} po síti. 
Tøídy, které z~Extension dìdí a~jsou prezentovány na návrhu robota uvedeném na obrázku \ref{picRobot}, jsou následující: \textit{Geoloc}, \textit{Tune}, \textit{Mood}, \textit{Activity} a~\textit{SwVersion}.
%Samotnou strukturou tìchto roz¹íøení, jak ji¾ bylo uvedeno vý¹e, jsou pomocí parametrù kopírovány vlastnosti jednotlivých XEP dokumentù. Tedy ke ka¾dému atributu XML zprávy existuje jak promìnná uchovávající její obsah, tak i tzv. \uv{get} a \uv{set} metoda. Pomocí dal¹í metody jsou jednotlivé èásti roz¹íøení rozparsovány, ulo¾eny do tøíd a následnì vlo¾eny do databází. 
Poslední doposud nezmiòované roz¹íøení v~podobì \textit{vcard}, je vyu¾ito z~knihovny gloox, která jej implementuje.

Základní èást, kterou je mo¾né pova¾ovat za jádro aplikace, je tøída \textit{Bot}. 
%Tato tøída je zalo¾ena na C++ knihovnì gloox. 
Je implementována pomocí tzv. \uv{handler} tøíd z~knihovny gloox, 
%Je implementována za pomocí vybraných tøíd, ze kterých dìdí. Jsou to pøedev¹ím \uv{handler} tøídy, 
kterými je zaji¹»ován pøístup k~jednotlivým zprávám. Neboli jejich prostøednictvím je získán pøístup k~samotným elementùm stanzy, jako je message, iq, a~presence. Konkrétní seznam tøíd, ze kterých je dìdìno, zobrazuje návrh robota na obrázku \ref{picRobot}. Jako pøíklad lze uvést tøídu, z~prostoru jmen gloox, \textit{VcardHandler}, díky ní¾ je mo¾né získat a~zpracovávat vcard u¾ivatelù, kteøí jsou v~seznamu kontaktù robota. Samotné pøidávání u¾ivatelù do seznamu kontaktù je provádìno automaticky. Pouze na stranì klienta je nutné po¾ádat robota o~autorizaci, která v¹ak probìhne automaticky. Jednotlivé kontakty nejsou ¾ádným zpùsobem tøídìny do skupin, tudí¾ existuje pouze jedna.

\begin{figure}[ht]
\begin{center}
\input{./pic/Robotsmall}
...
%\subsection*{Pøíkazy}
K~dal¹ím schopnostem robota, tedy kromì sbírání dat a~jejich následného ukládání do databáze, patøí reagovat na vybrané zprávy. Tyto zprávy jsou pøijímány pouze od u¾ivatelù, kteøí byli zaèlenìni do seznamu kontaktù tohoto robota. Základní pøíkaz, který nesmí chybìt u~¾ádné aplikace, je získání nápovìdy. V~první øadì informuje o~verzi robota a~jeho základních vlastnostech. Dále je odeslán struèný, ale výsti¾ný seznam pøíkazù i~s~jejich vysvìtlením. Pøi zaslání zprávy s~neznámým pøíkazem robotovi, je u¾ivateli vrácen struèný seznam kompatibilních pøíkazù. Pøehled tìchto pøíkazù se nachází v~pøíloze \ref{EPrikazy}.


\section{Pokraèování práce}
Obsah této podkapitoly je zamìøen na mo¾nosti roz¹íøení robota sbírajícího data. Také jsou zde popsány mo¾né zmìny, které se týkají u¾ivatelského rozhraní. V~závìru jsou shrnuta dal¹í roz¹íøení.

Vìt¹ina slu¾eb, které jsou dostupné na internetu a~jsou podporované rùznými servery, poskytuje pouze informace o~aktuálním stavu klienta. Pomocí vý¹e uvedených standardù by bylo mo¾né pøipojit k~základní webové prezentaci statusu i~informace roz¹íøené. Také zapojení standardu \textit{Entity Capabilities} \cite{Xep0115}, který je popsán na konci druhé kapitoly, by mohlo pøinést nové pokroèilej¹í, u¾iteèné informace. Konkrétnì je to samotná podpora IM aplikací v~oblasti dal¹ích XEP dokumentù. Nezasvìcený u¾ivatel by tímto velice jednoduchým roz¹íøením dokázal zjistit seznam slu¾eb podporovaných jeho IM programem. A» u¾ by se jednalo o~seznam funkcí, které jsou pouze pøijímány, nebo výèet roz¹íøení, k~jejich¾ odbìru je zaregistrován. Jako pøíklad lze uvést zji¹tìní schopnosti, a» u¾ pøijímání nebo odesílání informací o~pøehrávané hudbì popsané dokumentem \textit{User Tune} \cite{Xep0118}.

Dal¹í oblast, ve které lze nalézt mo¾nost roz¹íøení, se také zabývá webovou prezentací. Nyní v¹ak jde ji¾ konkrétnì o~robota vytvoøeného k~úèelu této práce. Informace potøebné k~zobrazení aktuálního stavu klienta na internetové stránce, jsou ulo¾eny v~databázi v~tabulce status. Tato tabulka je automaticky generována z~obsahu jiné entity, která obsahuje údaje o~v¹ech pøipojeních daného u¾ivatele. V¹e pracuje správnì a¾ do té chvíle, kdy u¾ivatel svou IM aplikaci neukonèí korektnì. Nastává situace, kdy na server není zaslána zpráva, která informuje o~zmìnì statusu. Díky tomuto neoèekávanému ukonèení klienta je u¾ivatel prezentován, jako by byl pøipojen i~kdy¾ je tomu naopak. Zdrojová tabulka se v~této chvíli stává neaktuální. Navrhované øe¹ení této situace vyu¾ívá roz¹íøení \textit{XMPP ping} \cite{Xep0199}, díky nìmu¾ by se mìlo dát zjistit, zda je u¾ivatel stále dostupný nebo ji¾ nikoliv.

V~oddílu robota by bylo mo¾né pokraèovat v~èásti, která se zaobírá pøíkazy zaslané ze strany u¾ivatele. S~tímto roz¹íøením úzce souvisí rozèlenìní u¾ivatelù do jednotlivých skupin. Ka¾dá takováto èást seznamu kontaktù by vlastnila rùzná práva, která by se stala základem pro poskytování konkrétních slu¾eb. Zaøazení do tìchto skupin by mohlo probíhat pøi ¾ádosti o~poèáteèní autorizaci, která by obsahovala zprávu s~pøesnì definovaným øetìzcem. Funkce, která je ji¾ uvedena vý¹e a~to v~podobì roz¹íøení webové prezentace, by také mohla být poskytována prostøednictvím robota. Konkrétnì se jedná o~zji¹tìní podporovaných a» ji¾ pøijímaných nebo odesílaných roz¹íøení. U¾ivatel by pouze poslal robotovi striktnì pøeddefinovanou zprávu a~jako odpovìï by obdr¾el seznam podporovaných funkcí.

Také èást zabývající se dolováním z~dat skýtá velké mno¾ství mo¾ností pro její pokraèování. Jako jeden z~pøíkladù lze uvést roz¹íøení zalo¾ené na datech získaných prostøednictvím dokumentu User Tune. Konkrétnì se jedná o~internetové stránky, které by také mimo jiné nabízely prodej hudby. Jako vzor lze brát stránku \textit{Last.fm} \cite{Last}, která jednotlivým u¾ivatelùm vytváøí hudební profily na základì poslouchaných skladeb. Navrhované roz¹íøení by získávalo data pro tvorbu profilu z~informací, které by byly ¹íøeny prostøednictvím sítì Jabber a~dokumentu User Tune. Vyu¾ití dal¹ího standardu User Location by nahradilo manuální nastavení geografické polohy u¾ivatele za automatické a~tím by pøineslo mo¾nosti pro nabízení cílené reklamy.

S~dokumentem User Location souvisí i~dal¹í navrhované pokraèování. Konkrétnì se jedná o~mo¾ný návrh nového XEP standardu, který by vyu¾íval urèení geografické polohy napøíklad k~naplánování cesty. Roz¹íøení by umo¾òovalo konkrétním lokalizaèním souøadnicím pøiøazovat dùle¾ité body, informace a~poznámky. Ty by byly ihned rozeslány zpùsobem, jakým pracují dosavadní roz¹íøení postavená na Personal Eventing Protocol \cite{Xep0060}. Vyu¾ití by nalezlo a» u¾ v~záchranných misích nebo jen k~¹íøení zajímavostí mezi u¾ivateli.

V~mìsíci bøezen tohoto roku vy¹la nová verze základních dokumentù RFC. V~této práci ji¾ ale z~tìchto standardù nejsou zahrnuty ¾ádné zmìny. V~dobì vydání se toti¾ práce dostávala do koneèné fáze vývoje. Proto zmìny pøinesené novými protokoly mohly být takté¾ uplatnìny pøi pøípadném pokraèování této práce.


\chapter{Vyhodnocení výsledkù}

Pátá kapitola je zamìøena na vyhodnocení výsledkù a~prezentaci zji¹tìných informací. V~první podkapitole jsou uvedeny poznatky zji¹tìné z~ruèního prozkoumávání získaných dat, která jsou ulo¾ena v~databázi. Z~velké èásti je zde popsán jak vznik nepøesností, tak i~jejich pøípadné následné odstranìní. V~závìru této podkapitoly jsou shrnuta kvantitativní fakta zamìøená na databázi jako je napøíklad poèet záznamù a~velikost samotné databáze.

Druhá podkapitola se zabývá samotným procesem dolováním z~dat, který je provádìn pomocí nástroje RapidMiner \cite{RapidMiner}. Je postavena na informacích získaných z~druhé a~tøetí kapitoly. Prostøednictvím tabulky \ref{tabProcent}, její¾ celá podoba se nachází v~pøíloze \ref{EVyhodnoceni}, jsou prezentovány zji¹tìné informace. Jedná se o~procentuální shodu u¾ivatelù mezi sebou, která je zalo¾ena na datech získaných z~jejich statusù v~Jabber síti.

\section{Manuální rozbor dat}
Data, která jsou získaná z~Jabber komunikace, jsou ulo¾ena v~databázi PostgreSQL. V prùbìhu celého vývoje této práce byl obsah databáze dùkladnì kontrolován a~pøípadné problémy øe¹eny v~nejbli¾¹ím mo¾ném termínu. I~tak se ale v~databázi nacházelo nìkolik chyb, které pøi následné analýze dokázaly mírnì znepøesnit výsledky. Jako pøíklad lze uvést zaznamenávání roz¹íøeného statusu o~geografické poloze do tabulky geoloc. V~pøípadì, ¾e u¾ivatel souèasnì pou¾íval pouze jedno pøipojení, bylo v¹e v~poøádku. Men¹í kolize nastala v~situaci, kdy bylo více souèasných pøipojení. Ka¾dé rozesílalo své informace, které byly stejné, a~tím se v~jeden èas v~databázi objevila redundantní data. Tato nepøesnost byla po manuálním rozboru dat objevena a~odstranìna pomocí èasových razítek.

Nejvìt¹í problém pøi sbírání dat je tvoøen díky aplikacím, které jsou vyu¾ívány u¾ivateli. Jejich neúplná implementace XEP dokumentù a~vzájemná nepøesná spolupráce s~jinými programy tvoøí velké mno¾ství matoucích dat, které se takté¾ nacházejí v~databázi. Tento problém se v~nejvìt¹í míøe objevuje v~tabulce tune, jejím¾ cílem je uchování informací o~hudbì, kterou u¾ivatelé poslouchají. Podle dokumentu User Tune \cite{Xep0118}, by po skonèení pøehrávání hudby mìl klient rozeslat do \uv{svìta} zprávu, její¾ obsah, nesoucí informace o~právì pøehrávané hudbì, bude prázdný. Tím je sdìleno ukonèení pøehrávání hudby. V~mnoha pøípadech se ale takto nedìje. Zprávu, která obsahuje informace o~poslední pøehrávané hudbì, u¾ivatel roze¹le jako poslední. Tudí¾ z~analýzy dat obsa¾ených v~databázi je mylnì pøedpokládáno, ¾e klient hudbu stále poslouchá.

Samotný sbìr informací zapoèal ji¾ v~minulém roce v~mìsíci prosinec. V~této dobì v¹ak je¹tì nebyla implementována v¹echna roz¹íøení  a~formát i~obsah vybraných tabulek byl pozdìji modifikován. Základní informace o~u¾ivatelském stavu, které jsou uchovávané v~tabulce presence, ji¾ ale výraznou zmìnou nepro¹ly a~tudí¾ jsou sbírány od samého poèátku bìhu aplikace. V~obsahu této entity pouze nejsou zahrnuty vybrané dny, kdy byl klient mimo provoz. 

Databáze uchovává informace pouze o~poètu kolem 40 u¾ivatelù. Tento fakt výraznì ovlivnil pøesnost a~rozsah nasbíraných dat. Takté¾ malé mno¾ství podpory roz¹íøených statusù ze strany u¾ivatelù a~jejich aplikací znaènì ovlivnilo obsah databáze. A» u¾ do poètu záznamù nebo umístìní informací do správných entit. S~tímto takté¾ úzce souvisí roz¹íøení User Tune, jeho¾ nesprávné pou¾ití je popsáno v~závìreèné èásti druhé kapitoly.

Za dobu, která pøesahuje 4 mìsíce, bylo nasbíráno témìø 200\,MB dat. Napøíklad tabulka, která uchovává data pro data mining popsaný v~následující podkapitole, obsahuje pøes 115 tisíc záznamù. Na druhou stranu entity zamìøené na roz¹íøené statusy, konkrétnì to jsou geoloc, tune, mood a~activity, disponují s~men¹ím poètem záznamù, které se poèítají pouze ve stovkách. Tento omezený poèet, jak ji¾ bylo uvedeno, je výsledkem men¹í podpory tìchto roz¹íøení ze strany klientù.

Tabulka \ref{tabSouhrn} souhrnnì prezentuje nasbíraná data v~jednotlivých kategoriích roz¹íøení. Prvními zji¹tìnými údaji je tvoøen druhý sloupec, kterým je shrnut poèet u¾ivatelù podporujících daný standard. Jak druhý tak i~tøetí sloupec se skládá ze dvou èísel, která jsou  vzájemnì oddìlena znakem lomítko \textit{\textfractionsolidus}. První údaj uvádí poèet u¾iteèných záznamù z~celkového poètu. Konkrétnì v~druhém sloupci to je poèet u¾ivatelù, kteøí ¹íøili u¾iteèné informace, k~poètu u¾ivatelù, je¾ ¹íøili jakákoliv data, tedy i~prázdná. Tabulku uzavírá pøehled prùmìrného poètu záznamù na jednoho u¾ivatele.

\begin{table}[ht]
\begin{center}
\begin{tabular}{ l l l l } 
\rowcolor[gray]{0.9}
\textbf{Roz¹íøení} 	& \textbf{U¾ivatelé/z} & \textbf{Záznamy/z} & \textbf{Prùmìr}\\
User Tune				&   10\,\%(4)/44\,\%(18)		& 2258/4789			& 564\\
User Location			&   12\,\%(5)/12\,\%(5) 		& 122/276			& 24\\
User Mood				&   20\,\%(8)/31\,\%(13) 	& 550/1367			& 63\\
User Activity			&   10\,\%(4)/20\,\%(8)		& 404/1136			& 101\\ \hline
\end{tabular}
\caption{Pøehled manuálního rozboru dat.}
\label{tabSouhrn}
\end{center} 
\end{table}

Manuální rozbor dat probíhal i~pøi tvorbì webové prezentace, která se zamìøuje na praktické vyu¾ití získaných dat. Lze ji nalézt na stránce \url{http://pcpanel-l205.fit.vutbr.cz/jabinfo/} na její¾ tvorbì se z~velké èásti podílel vedoucí této práce Ing. Jozef Mlích. Obsah této ukázky slou¾í i~jako názorný manuál, jak by se s~daty mohlo dále nakládat.

Následující podkapitola se zamìøuje na vyhodnocení údajù z~tabulky presence. Toto rozhodnutí je postaveno na ruèním prozkoumání získaných dat, které je shrnuto v~tabulce \ref{tabSouhrn}. Jsou-li pou¾ita data z~entit mood a~ativity je na první pohled patrné, ¾e u¾ivatelé vyu¾ívající tyto slu¾by, je ve své aplikaci nastavili pouze jedenkrát. Jeliko¾ nastavení tìchto roz¹íøení neprobíhá automaticky, nelze získané informace pova¾ovat za vìrohodné. Pro samotnou analýzu by byl potøebný mnohonásobnì vìt¹í objem dat, ne¾ jaký by byl nutný napøíklad  pro tabulky tune a~geoloc. Po prozkoumání tabulky geoloc bylo vyvozeno, ¾e získané geografické polohy od klientù nejsou dále pou¾itelné.

%USER TUNE

\section{Programové vyhodnocení}
V~této èásti vyhodnocení jsou vyu¾ita data získaná robotem z~Jabber komunikace, která jsou uschována v~databázi. Pøed samotným procesem dolování z~dat, na øadu pøichází transformace popsaná v~kapitole tøetí, která data pøipraví. Je vytvoøen formát, který je kompatibilní s~programem RapidMiner \cite{RapidMiner}. Jak bylo uvedeno na konci pøede¹lé kapitoly, roz¹íøené statusy pro proces dolování z~dat nejsou vhodné. I~z~tohoto dùvodu, ale hlavnì díky velkému mno¾ství nasbíraných dat byla pro data mining vyu¾ita tabulka presence, její¾ obsah je tvoøen informacemi o~zmìnì statusù jednotlivých u¾ivatelù.

Struktura samotného procesu vytvoøeného v~programu RapidMiner je zobrazena na obrázku \ref{picProces}. Jak je patrné je tvoøena z~nìkolika blokù, kde ka¾dý plní urèitou funkci. Vzájemné vztahy mezi jednotlivými oddíly procesu jsou reprezentovány úseèkami. První èástí, která je nazvána \textit{Read CSV}, jsou transformovaná data naètena z~externího souboru do nástroje. Následující oddíl \textit{Clustering} vyu¾ívá algoritmus \textit{k}-means, kterým se podrobnì zabývá kapitola tøetí. Poèet shlukù byl experimentováním nastaven na koneènou hodnotu \textit{100}. 
\begin{figure}[ht]
\begin{center}
\includegraphics[scale=.6]{pic/processmaly.pdf}
\caption{Struktura procesu v~nástroji RapidMiner.}
 \label{picProces}
\end{center}
\end{figure}
Blok nazvaný \textit{SVD}\footnote{Singular Value Decomposition} slou¾í ke sní¾ení dimenzí jednotlivých vstupních vektorù. Pro vizuální zobrazení bylo zvoleno sní¾ení dimenzí z~288 a¾ na hodnotu 3. Následujícím oddílem \textit{Select Attribute}, který vyu¾ívá data z~bloku Clustering, jsou vybrány pouze dva atributy. Konkrétnì to jsou jednotlivá JID u¾ivatelù a~jména shlukù, do kterých byly zaøazeny. Posledním blokem jsou tato data exportována do CSV souboru.

%U programù, které vyu¾ívají vizuální blokové sestavení procesù je nastavení jednotlivých parametrù velice jednoduché. A» u¾ to je nastavení vstupního nebo výstupního souboru ve tvaru CSV nebo ji¾ zmiòované nastavení poètu shlukù u metody \textit{k}-means.

\begin{figure}[ht]
\begin{center}
\includegraphics[scale=.60]{pic/plot3d1.pdf}
\caption{Výsledné shluky pøevedeny do 3D prostoru.}
 \label{picPlot}
\end{center}
\end{figure}

Obrázkem \ref{picPlot} je prezentováno rozmístìní jednotlivých shlukù v~tøí rozmìrném prostoru. Byl vygenerován automaticky blokem SVD zobrazeném na obrázku \ref{picProces}. Tento blok, jak ji¾ bylo uvedeno, redukuje \textit{n}-rozmìrný vstupní vektor do tøí dimenzí. Sni¾ování prostorového øádu bylo provedeno za úèelem grafického znázornìní jednotlivých shlukù. Tyto výsledné mno¾iny jsou od sebe navzájem odli¹eny barvami.

Pomocí algoritmu \textit{k}-means byli u¾ivatelé rozèlenìni podle svého chování do nìkolika mno¾in. Pouze jedenáct shlukù obsahovalo více jak osmdesát záznamù a~právì ty jsou prezentovány na obrázku \ref{picShluk}. Je zde znázornìn graf znázoròující vztah mezi statusem a~èasem. Osa \textit{x} je tvoøena èasovým intervalem v~rozmezí jednoho dne a~na ose \textit{y} je vynesen aktuální stav neboli presence.

\begin{figure}[ht]
\begin{center}
\includegraphics[scale=.75]{./pic/g-finally.pdf}
\caption{Prùbìh statusù shlukù podle èasu.}
 \label{picShluk}
\end{center}
\end{figure}

Po provedení shlukování algoritmem \textit{k}-means pomocí nástroje RapidMiner jsou u¾ivatelé rozøazeni do shlukù a~ulo¾eni do externího souboru. V~této chvíli se celkové vyhodnocení dostává do závìreèné fáze. Na øadu pøichází aplikace, pomocí které je vypoèítáno zastoupení jednotlivých u¾ivatelù ve shlucích.

Vzájemná procentuální shoda u¾ivatelù mezi sebou je prezentována tabulkou \ref{tabProcent}. Kde hlavièka, která je podbarvena ¹edì a~umístìna na prvním øádku, znázoròuje vybrané u¾ivatele. Jejich samotné JID je, za úèelem úspory místa a~zachování soukromí, nahrazeno èísly od jedné po dvaadvacet. Tato struktura je uvedená i~v~prvním sloupci. Je tedy vytvoøena \uv{matice}, její¾ hlavní diagonála není vyplnìna. Obsah je tvoøen procentuální shodou mezi jednotlivými u¾ivateli. Buòky prezentující procentuální zastoupení vìt¹í ne¾ 80\,\% jsou barevnì odli¹eny. Zelenì zabarvená políèka znázoròují podobnost v~intervalu od 80 do 89 procent a~èervená barva znaèí interval shody mezi 90 a¾ 99.
\begin{table}[ht]
 \subtable{
 \centering
 \renewcommand{\arraystretch}{1.2}
  {\tiny\tt
\begin{tabular}{ >{\columncolor[gray]{.9}}l c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c c} 
\rowcolor[gray]{0.9}
\textbf{-} &\textbf{1} 	& \textbf{2} & \textbf{3} & \textbf{4}& \textbf{5}& \textbf{6}& \textbf{7}& \textcolor{green}{8}& \textbf{9}& \textbf{10}& \textbf{11}& \textbf{12} & \textbf{13}& \textbf{14}&\textbf{15}&\textbf{16}&\textbf{17}&\textbf{18}&\textbf{19}&\textbf{20}&\textbf{21}&\textbf{22} \\
1&-&22&31&70&23&28&31&37&55&54&64&14&52&32&34&54&44&27&23&19&29&23\\
2&22&-&63&24&\textcolor{red}{90}&\textcolor{green}{88}&84&55&52&39&34&\textcolor{red}{90}&29&14&56&32&44&\textcolor{green}{89}&\textcolor{green}{85}&50&33&22\\
...
5&23&\textcolor{red}{90}&63&23&-&\textcolor{red}{90}&\textcolor{green}{85}&60&49&42&34&\textcolor{green}{88}&32&17&55&30&42&\textcolor{green}{86}&\textcolor{green}{86}&49&34&23\\
6&28&\textcolor{green}{88}&66&29&\textcolor{red}{90}&-&\textcolor{green}{86}&60&56&44&38&\textcolor{green}{85}&34&17&58&36&42&\textcolor{green}{87}&\textcolor{green}{85}&49&32&24\\
7&31&84&64&30&\textcolor{green}{85}&\textcolor{green}{86}&-&59&53&49&43&\textcolor{green}{81}&31&22&56&32&42&\textcolor{green}{86}&\textcolor{green}{83}&51&34&25\\
...
12&14&\textcolor{red}{90}&60&14&\textcolor{green}{88}&\textcolor{green}{85}&\textcolor{green}{81}&51&43&33&26&-&24&9&51&25&36&\textcolor{green}{82}&\textcolor{green}{82}&49&29&21\\
...
18&27&\textcolor{green}{89}&66&26&\textcolor{green}{86}&\textcolor{green}{87}&\textcolor{green}{86}&58&56&44&38&\textcolor{green}{82}&37&17&59&34&49&-&\textcolor{green}{86}&51&36&23\\
19&23&\textcolor{green}{85}&63&21&\textcolor{green}{86}&\textcolor{green}{85}&\textcolor{green}{83}&59&52&42&34&\textcolor{green}{82}&29&14&56&30&46&\textcolor{green}{86}&-&51&36&25\\
...
22&23&22&38&19&23&24&25&24&26&29&29&21&30&19&44&25&25&23&25&35&26&-\\ \hline
\end{tabular}} }
\caption{Procentuální shoda jednotlivých u¾ivatelù.}
\label{tabProcent}
\end{table}

Vý¹e zmínìná tabulka \ref{tabProcent} prezentuje pouze vybranou èást u¾ivatelù. Celý pøehled je umístìn v~pøíloze \ref{EVyhodnoceni} v~tabulce \ref{ETabProcent}. Jednotlivé nahrazení JID u¾ivatelù za èísla je takté¾ uvedeno v~pøíloze \ref{EVyhodnoceni}, konkrétnì v~tabulce \ref{ETabPrirazeni}.

Údaje prezentované tabulkou \ref{tabProcent} byly i~manuálnì zkontrolovány a~to vedlo k~závìru, ¾e celý proces poèínaje transformací a¾ po samotné vyhodnocení je navr¾en správnì. Je patrné, ¾e ke 100 procentní shodì nedo¹lo ani u~u¾ivatelù, kteøí vyu¾ívají dva stejné úèty. Tento dùsledek je mo¾né vysvìtlit tím, ¾e ne v¾dy byly oba úèty ve stejném stavu a~ve stejné chvíli zapínány. Takté¾ se dospìlo ke zji¹tìní, které dokazuje, ¾e ke správnému vyhodnocení by bylo potøeba, aby v¹ichni u¾ivatelé mìli pøibli¾nì stejný poèet záznamù obsahujících zmìnu statusu. K~velké procentuální shodì napøíklad do¹lo u~dvou klientù, kteøí mìli v~databázi pouze tøi záznamy, co¾ nemá ¾ádnou vypovídající hodnotu o~jejich podobnosti.

\chapter{Závìr}
Cílem této bakaláøské práce bylo vytvoøení klienta pro sí» Jabber pracující v~reálném èase. Tato aplikace pracuje jako robot, který je schopen reagovat na vhodnì zvolené pøíkazy a~sbírat data z~komunikace. Následujícím krokem bylo analyzování nashromá¾dìných dat, se kterými byl dále proveden proces dolování. Výsledky jsou prezentovány pomocí tabulky, její¾ obsah je tvoøen procentuální shodou jednotlivých u¾ivatelù mezi sebou.

Po dùkladném vypracování této práce je mo¾né prohlásit, ¾e v¹echny cíle kladené na tuto práci se podaøilo splnit. Ka¾dému bodu zadání je zde vìnován oddíl, ve kterém je daná problematika podrobnì rozebrána. Prvnímu bodu zadání odpovídá celá druhá kapitola, která je tvoøena popisem protokolu XMPP, na kterém je postavena komunikaèní slu¾ba Jabber. Také je zde rozebrána architektura sítì a~základní stavební kameny XMPP protokolu, které jsou vyu¾ívány sítí Jabber jako nástroj ke zprostøedkování jednotlivých slu¾eb. Druhý bod zadání úzce souvisí s~tvorbou robota, který byl vytvoøen za úèelem získávání dat z~Jabber komunikace. Zdrojové soubory této aplikace jsou pøilo¾eny na kompaktním disku. Implementaèní èástí se vìnuje kapitola ètvrtá, kde je rozebrána jak celková programová architektura této práce, tak její jednotlivé bloky. Prvním oddílem kapitoly páté je splnìn tøetí bod zadání. Jsou zde uvedeny poznatky, které byly získány z~manuálního prùzkumu dat. Z~ruèního prohlí¾ení dat byly vyvozeny dùsledky, které tvoøily základ pro splnìní bodu ètvrtého. Tomuto a~následnému bodu zadání je vìnována zbývající èást páté kapitoly, která je doplnìna o~pøílohy nacházející se na konci tohoto dokumentu. Pro automatické zkoumání dat byl vyu¾it nástroj RapidMiner, který je spoleènì s~dal¹ími vybranými programy popsán na konci kapitoly tøetí. Její obsah je zamìøen na popis procesu dolování z~dat a~na jeho zaèlenìní do celého bloku získávání znalostí z~databází. Poslednímu bodu zadání je vyhrazena èást v~kapitole ètvrté, která se zabývá pøípadným pokraèováním této práce.

Jak je uvedeno vý¹e, ve¹keré body zadání byly splnìny. Jako roz¹íøení zadání bylo zpøístupnìní mo¾nosti vyu¾ít jak robota tak i~data ulo¾ená v~databázi pro prezentování vlastního statusu na webových stránkách. Pøíklad tohoto pou¾ití je pøilo¾ený na kompaktním disku a~také je k~dispozici k~nahlédnutí na internetové stránce \url{http://pcpanel-l205.fit.vutbr.cz/jabinfo/}.

Pro nedostateèný poèet u¾ivatelù, kteøí doposud vyu¾ívali tohoto robota, a~zároveò by mìli správnì nastavené \uv{rozesílání} roz¹íøených statusù vedlo k~následujícímu rozhodnutí. Vìt¹í dùraz byl kladen na vývoj samotného robota a~sbírání i~ukládání dat do databáze, ne¾ na následný proces dolování z~dat. Za období ètyø mìsícù bylo celkem nasbíráno kolem 200\,MB dat a~pøes 100 tisíc záznamù.