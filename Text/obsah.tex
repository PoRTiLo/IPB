...
Dobøe navr¾ená internetová technologie je tvoøe správnì fungujícími komponenty, které mezi sebou doká¾í vytvoøit spojení a následnì zapoèít komunikaci. Pro popis Jabber architektury v této práci bylo èerpáno z \cite{Adams,Saint}. 
...
Pøi výbìru velikosti jejich hodnoty bylo èerpáno z programu Gajim, který je multiplatformní klient s velkou podporou standardù a roz¹iøujících protokolù. Bylo èerpáno právì z jeho funkce, která podle èasových intervalù neèinnosti u¾ivatele, automaticky nastavuje status a hlavnì èíslo priority. Jeliko¾ rozsah mezi hodnotami priorit v této aplikaci mi pøi¹el jako nedostateèný, má zvolená vý¹e priorit pouze dodr¾uje poøadí jednotlivých velikostí. Na druhou stranu prvky ze vstupní mno¾iny $S$ jsou striktnì definovány podle Jabber standardu, který je popsán v RFC \cite{Rfc3920}.
...
Struktura databáze, do které je ukládána ve¹kerá komunikace Jabber robota, vyu¾ívá relaèní model. Obrázkem è. \ref{picDatabase} jsou prezentovány nejdùle¾itìj¹í èásti databáze. Celou strukturu návrhu je mo¾né nalézt v pøíloze \ref{Edatabase}. V jednotlivých èástí návrhu databáze je poèítáno s jejím druhotným vyu¾itím obsahu, které bude popsáno v dal¹ích èástí této práce.

%temporalni DB
V dobì návrhu databáze zcela nebylo jasné, která data budou následnì analyzována. Z toho dùvodu se struktura databáze sna¾í zachytit v¹echna \uv{dùle¾itá} data. Pøedem nebylo urèen typ znalostí, kterých by se pøi dolování mìlo dosáhnout. Za úèelem neztratit ¾ádná data, je v návrhu databáze obsa¾ena tabulka \textit{xml}, která je nositelem obsahu jak v¹ech pøijatých, tak i odeslaných zpráv. Tabulky \textit{debug}, \textit{level} a \textit{logarea}, které v zú¾eném návrhu databáze è. \ref{picDatabase} nejsou zobrazeny, jsou urèeny pouze jako doplòkové informace k typu zprávy. Pøíkladem mù¾e být øádek v tabulce xml s \textit{area} èíslem 32, kterým jsou oznaèeny zprávy spojeny s pøekladem IP adres a doménových jmen. Hodnota \textit{level} je pro vìt¹inu zpráv nastavena na hodnotu nula, co¾ znamená ¾e nejde o zprávu obsahující chybu ani varování. Z pohledu návrhu vytváøí tabulka \textit{xml} redundantní data, dalo by se øíct, ¾e je zbyteèná, ¾e je nahrazena jednotlivými dal¹ími tabulkami, které jsou zamìøeny na konkrétní data. Pøíkladem je tabulka \textit{message}, která je takté¾ vyobrazena pouze v pøíloze. Jejím obsahem jsou zprávy vzniklé pøi Jabber komunikace mezi u¾ivatelem a robotem, jeho¾ schopnosti budou popsány ní¾e.

Tabulka \textit{presence} z pohledu XMPP standardu prezentuje jeden ze tøí základních èástí stanzy, element \textit{presence}. Základním cílem této tabulky je shroma¾ïování u¾ivatelských statusù. Tento po¾adavek je splnìn atributem \textit{presence}. Jak ji¾ bylo zmínìno ve tøetí kapitole, v èásti která se zabývá transformací, atribut presence obsahuje hodnoty typu text. Pøíklad v¹ech mo¾ných hodnot, kterých tento atribut mù¾e nabývat je prezentován v pøíloze \ref{Estanza} v èásti zabývající se elementem presence. K dal¹ím atributùm této tabulky patøí \textit{message}, který je reprezentován textovým øetìzcem a roz¹iøuje informace o stavu. V této èásti je èasto obsa¾en text, který je do statusu pøidán automaticky IM klientem. Jako pøíklad poslou¾í vìta \uv{I'm not here right now} generována aplikací Miranda po del¹ím èasovém úseku neèinnosti.

Vìt¹ina roz¹íøení standardu XMPP, která jsou popsána ve druhé kapitole, jsou reprezentována pomocí pìti tabulek. Konkrétnì to jsou tabulky \textit{geoloc}, \textit{tune}, \textit{mood}, \textit{activity} a \textit{vcard}. Poslední roz¹íøení informující o verzi a jménu IM klienta a systému bude rozebráno v následujícím odstavci v souvislosti s dal¹í tabulkou \textit{resource}. Pro obsah a názvy atributù v¹ech pìti tabulek s roz¹íøeními se staly vzory dokumenty jednotlivých XEP, které je definují. Podrobnìj¹í popis v¹ech atributù lze tedy nalézt právì v nich. Tabulky kromì tìchto atributù obsahují také polo¾ku \textit{jidbare}, která, jak ji¾ název napovídá, obsahuje pouze èisté JabberID bez resources. Tato skuteènost vyplývá ji¾ ze samotného návrhu XEP protokolù. Adresa u¾ivatele, který posílá serveru zprávu s jedním z mo¾ných roz¹íøení, je také pouze v základním stavu. V tabulce \textit{vcard} jsou také i polo¾ky, její¾ obsah tvoøí fotografie. Tyto atributy ale nejsou binárního typu nýbr¾ textového. Obrázky jsou zde ulo¾eny, tak jak jsou i pøená¹eny po síti, tedy pomocí datového formátu Base64. Base64 je algoritmus, který pøevádí binární data na øetìzec znakù tabulky ASCII.

S posledním roz¹íøením nazvaném \textit{Software version} je spjata tabulka resource. Její nìkolik sloupcù tvoøí právì informace o IM klientovi a operaèním systému, na kterém bì¾í. Tyto základní údaje jsou definovány v protokolu \cite{Xep0092}, kde je také mo¾né èerpat bli¾¹í informace. Pokroèilej¹í atributy, jako je \textit{type} a \textit{osversion} nacházejí svùj podklad v XEP dokumentu \cite{Xep0115} takté¾ jako atribut \textit{ver}, který je podrobnìji popsán ve druhé kapitole této práce. V neposlední øadì se zde nachází zmínka o podpoøe audia a videa a» u¾ v podobì \textit{jingle} roz¹íøené nebo \textit{google}. tyto sloupce jsou typu boolean, tudí¾ hodnota \textit{true} znamená podporu daného sloupce. Pøesto tyto vý¹e uvedené údaje nejsou hlavním dùvodem existence této tabulky. Její primární cíl je uchovávat informace o v¹ech pøipojených u¾ivatelích aat.

Poslední zde rozebra
neni jehi hlavni funkce.....sdfsdf.f..
\begin{figure}[ht]
\begin{center}
\input{DatabaseSmall}
