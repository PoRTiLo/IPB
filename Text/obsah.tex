...
Shlukování pomocí algoritmu \textit{k}--means je pou¾íváno pro data obsahující kvantitativní promìnné a pro data, která nejsou pøíli¹ za¹umìlá. Základní proces je tvoøen iterativním rozdìlováním objektù do tøíd na základì vzdáleností od jejich støedù. Støed nebo--li centroid shluku je vektor, jeho¾ vzdálenost od souètu vzdáleností objektù v této tøídì je minimální. Pro výpoèet vzdáleností mezi objekty samotnými nebo mezi objekty a støedem je pou¾ita euklidovská vzdálenost\footnote{mean=støed, centroid je vektor prùmìrù}, která je vyobrazena na obrázku è. \ref{picStat}. 
...
%  \begin{tikzpicture}
 ...
 %   bend angle=45,
 %   pre/.style={<-, shorten <=1pt,>=stealth', semithick, minimum size=4mm},
 %   post/.style={->, shorten >=1pt, >=stealth', semithick,  minimum size=4mm}]
 %   \node at ( 0,0) [place, label=above:$1.krok$] (p0){};
 %   \node at ( 3,0) [place, label=above:$2.krok$] (p1){}
 %    edge [pre] (p0);
  %  \node at ( 6,0) [place, label=above:$3.krok$] (p2){}
  %    edge [pre] (p1)
  %    edge [post, bend right] (p1);
  %  \node at ( 9,0) [place, label=above:$4.krok$] (p3){}
   %   edge [pre] (p2);
   \begin{tikzpicture}[->,>=stealth',shorten >=1pt,%
		      auto,node distance=2cm,on grid,semithick,
		      inner sep=2pt,bend angle=45]
    \node at ( 0,0) [initial,state, label=above:$1.krok$] (A){};
    \node at ( 3,0) [state, label=above:$2.krok$](B)  {};
    \node at ( 6,0) [state, label=above:$3.krok$](C)  {};
    \node at ( 9,0) [state, accepting, label=above:$4.krok$](D)  {};
