...
Pátá kapitola se zamìøuje na vyhodnocení výsledkù a prezentaci zji¹tìných informací. V první podkapitole jsou uvedeny poznatky zji¹tìné z ruèního prozkoumávání získaných dat, která jsou ulo¾ena v databázi. Pøevá¾nì je zde popsán jak vznik nepøesností, tak jejich pøípadné následné odstranìní. V závìru této podkapitoly jsou shrnuta kvantitativní fakta zamìøená na databázi, jako je napøíklad poèet záznamù a velikost samotné databáze.

Druhá podkapitola se zabývá samotným data miningem, který je provádìn pomocí nástroje RapidMiner \cite{RapidMiner}. Je postavena na informacích získaných z druhé a tøetí kapitoly. 

\section{Manuální rozbor dat}
Data, která jsou získaná z Jabber komunikace, jsou ulo¾ena v databázi PostgreSQL. V prùbìhu celého vývoje této práce byl obsah databáze dùkladnì kontrolován a pøípadné problémy øe¹eny v nejbli¾¹ím mo¾ném termínu. I tak se ale v databázi nachází nìkolik chyb, které pøi následné analýze doká¾í mírnì znepøesnit výsledky. Jako pøíklad lze uvést zaznamenávání roz¹íøeného statusu o geografické poloze do tabulky Geoloc. V pøípadì, ¾e u¾ivatel souèasnì pou¾ívá pouze jedno pøipojení, je v¹e v poøádku. Men¹í kolize nastala v pøípadì, kdy bylo více souèasných pøipojení. Ka¾dé rozesílalo své stejné informace a tím se v jeden èas v databázi objevila redundantní data. Tato nepøesnost byla po manuálním rozboru dat objevena a vyøe¹ena pomocí èasových razítek.

Snad nejvìt¹í problém pøi sbírání dat, je tvoøen díky aplikacím, které jsou vyu¾ívány u¾ivateli. Jejich neúplná implementace XEP dokumentù a vzájemná nepøesná komunikace s jinými programy tvoøí velké mno¾ství matoucích dat, která se takté¾ nacházejí v databázi. Tento problém se v nejvìt¹í míøe objevuje v tabulce Tune, jejím¾ cílem je uchování informací o hudbì, kterou u¾ivatelé poslouchají. Podle dokumentu User Tune \cite{Xep0118}, by po skonèení pøehrávání hudby mìl klient rozeslat do \uv{svìta} zprávu, její¾ obsah, nesoucí informace o právì pøehrávané hudbì, bude prázdný. Tím je sdìleno ukonèení pøehrávání hudby. V mnoha pøípadech se, ale takto nedìje. Zpráva obsahující informace o poslední pøehrávané hudbì je to co u¾ivatel roze¹le jako poslední. Tudí¾ z analýzy dat obsa¾ených v databázi je mylnì pøedpokládáno, ¾e u¾ivatel hudbu stále poslouchá.

Samotný sbìr informací zapoèal ji¾ minulého roku v mìsíci prosinec. V této dobì v¹ak je¹tì nebyla implementována v¹echna roz¹íøení  a formát i obsah vybraných tabulek byl pozdìji modifikován. Základní informace o stavu u¾ivatele, uchovávaných v tabulce presence, ji¾ ale výraznou zmìnou nepro¹ly a tudí¾ jsou sbírány od samého poèátku bìhu aplikace. V obsahu této entity pouze nejsou zahrnuty vybrané dny, kdy byl klient mimo provoz. 
...
Manuální rozbor dat probíhal i pøi tvorbì webové prezentace, která se zamìøuje na praktické vyu¾ití získaných dat. Je umístìna na stránce \url{http://pcpanel-l205.fit.vutbr.cz/jabinfo/} na její¾ tvorbì se z velké èásti podílel vedoucí této práce Ing. Jozef Mlích. Obsah této stránky slou¾í i jako názorný manuál jak by se s daty mohlo dále nakládat.

Následující podkapitola se zamìøuje na vyhodnocení údajù z tabulky presence. Toto rozhodnutí je postaveno na ruèním prozkoumání získaných dat, které je shrnuto v tabulce \ref{tabSouhrn}. Jsou-li pou¾ita data z entit mood a ativity je na první pohled patrné, ¾e u¾ivatelé vyu¾ívající tyto slu¾by, je ve své aplikaci nastavili pouze jedenkrát. Jeliko¾ nastavení tìchto roz¹íøení neprobíhá automaticky, nelze získané informace pova¾ovat za vìrohodné. Pro samotnou analýzu by tedy bylo potøeba mnohem více dat, ne¾ kolik by jich bylo potøeba pro tabulky tune a geoloc. Po prozkoumání tabulky geoloc bylo vyvozeno, ¾e získané geografické polohy od klientù není dále pou¾itelné.

%USER TUNE

\section{Programové vyhodnocení}
Jsou zde vyu¾ita  data získaná robotem z Jabber komunikace, která jsou uschována v databázi. Pøed samotným procesem dolování z dat, na øadu pøichází transformace popsaná v kapitole tøetí, která data pøipraví. Je vytvoøen formát, který je kompatibilní s programem RapidMiner \cite{RapidMiner}. Jak bylo uvedeno na konci pøede¹lé kapitoly, roz¹íøené statusy pro proces dolování z dat nejsou vhodné. I z tohoto dùvodu, ale hlavnì díky velkému mno¾ství nasbíraných dat byla pro data mining vyu¾ita tabulka presence, její¾ obsah je tvoøen informacemi o zmìnì statusù jednotlivých u¾ivatelù.

Struktura samotného procesu vytvoøeného v programu RapidMiner je zobrazena na obrázku \ref{picProces}. Jak je patrné je tvoøen z nìkolika blokù, kde ka¾dý plní urèitou funkci. Vzájemné vztahy mezi jednotlivými oddíly procesu jsou reprezentovány úseèkami. První èástí, která je nazvána \textit{Read CSV}, jsou transformovaná data naètena z externího souboru. Následující oddíl \textit{Clustering} vyu¾ívá algoritmus \textit{k}-means, kterým se podrobnì zabývá kapitola tøetí. Poèet shlukù byl experimentováním nastaven na koneènou hodnotu \textit{100}. 
\begin{figure}[ht]
\begin{center}
...
\end{center}
\end{figure}
Blok nazvaný SVD\footnote{Singular Value Decomposition} slou¾í je sní¾ení dimenzí jednotlivých vektorù. Pro vizuální zobrazení bylo zvoleno sní¾ení dimenzí z 288 a¾ na hodnotu 3. Následujícím oddílem, který je spojen s blokem SVD, ale vyu¾ívané data vycházejí z bloku Clustering, jsou vybrány pouze dva atributy. Konkrétnì to jsou jednotlivá JID u¾ivatelù a jména shlukù, do kterých byly zaøazeny. Posledním blokem jsou tato data exportována do SCV souboru.

%U programù, které vyu¾ívají vizuální blokové sestavení procesù je nastavení jednotlivých parametrù velice jednoduché. A» ji¾ to je nastavení vstupního nebo výstupního souboru ve tvaru CSV nebo ji¾ zmiòované nastavení poètu shlukù u metody \textit{k}-means.

\begin{figure}[ht]
\begin{center}
\includegraphics[scale=.70]{pic/plot3d1.pdf}
\caption{Výsledné shluky pøevedeny do 3D prostoru.}
 \label{picPlot}
\end{center}
\end{figure}

Obrázkem \ref{picPlot} je prezentováno rozmístìní jednotlivých shlukù v tøí rozmìrném prostoru. Byl vygenerován automaticky blokem SVD zobrazeném na obrázku \ref{picProces}. Tento blok, jak ji¾ bylo øeèeno, redukuje \textit{n}-rozmìrný vstupní vektor do tøí dimenzí. Sni¾ování prostorového øádu bylo provedeno za úèelem grafického znázornìní jednotlivých shlukù. Tyto výsledné mno¾iny jsou od sebe navzájem odli¹eny barvami.

Pomocí algoritmu \textit{k}-means byli u¾ivatelé rozèlenìni podle svého chování do nìkolika mno¾in. Pouze jedenáct shlukù obsahovalo více jak osmdesát záznamù. Právì tìchto jedenáct shlukù je prezentováno na obrázku \ref{picShluk}. Je zde znázornìn graf znázoròující vztah mezi statusem a èasem. Osa \textit{x} je tvoøena èasovým intervalem v rozmezí jednoho dne a na ose \textit{y} je vynesen aktuální stav neboli presence.
...
Vzájemné procentuální shoda u¾ivatelù mezi sebou je prezentována tabulkou \ref{tabProcent}. Kde hlavièka, která je podbarvena ¹edì a umístìna na prvním øádku, znázoròuje vybrané u¾ivatele. Jejich samotné JID je, za úèelem u¹etøení místa a udr¾ení soukromí, nahrazeno èísly od jedné po dvaadvacet. Tato struktura je uvedená i v prvním sloupci. Je tedy vytvoøena \uv{matice}, její¾ hlavní diagonála není vyplnìna. Obsah tvoøí procentuální shoda mezi jednotlivými u¾ivateli. Buòky prezentující procentuální shody vìt¹í ne¾ 80\,\% jsou barevnì odli¹eny. Zelenì zabarvená políèka znázoròují shodu v intervalu od 80 do 89 procent a èervená barva znaèí interval shody mezi 90 a¾ 99.
\begin{table}[ht]
 \subtable{
 \centering
...
%& \textbf{22} & \textbf{23} & \textbf{24}& \textbf{25}& \textbf{26}& \textbf{27}& \textbf{28}& \textbf{29}& \textbf{30}&\textbf{31} & \textbf{32} & \textbf{33} & \textbf{34}& \textbf{35}& \textbf{36}& \textbf{37}& \textbf{38}& \textbf{39}&\textbf{40}&\textbf{41}\\
...
Vý¹e zmínìná tabulka \ref{tabProcent} prezentuje pouze vybranou èást u¾ivatelù. Celé pøehled je umístìn v pøíloze \ref{EVyhodnoceni} v tabulce \ref{ETabProcent}. Jednotlivé nahrazení JID u¾ivatelù za èísla je takté¾ uvedeno v pøíloze \ref{EVyhodnoceni}, pøesnìji v tabulce \ref{ETabPrirazeni}.

Údaje prezentované tabulkou \ref{tabProcent} byly i manuálnì zkontrolovány a to vedlo k závìru, ¾e celý proces poèínaje transformací a¾ po samotné vyhodnocení je navr¾en správnì. Je patrné, ¾e ke 100 procentní shodì nedo¹lo ani u u¾ivatelù, kteøí pou¾ívají dva stejné úèty. Tento dùsledek je mo¾né vysvìtlit  tím, ¾e ne v¾dy byly oba úèty ve stejném stavu. Takté¾ dospìlo ke zji¹tìní, které dokazuje, ¾e ke správnému vyhodnocení by bylo potøeba, aby v¹ichni u¾ivatelé mìli pøibli¾nì stejný poèet záznamù obsahujících zmìnu statusu. K velké procentuální shodì napøíklad do¹lo u dvou klientù, kteøí ale mìli v databázi pouze tøi záznamy.
\chapter{Závìr}


Po dùkladném vypracování této práce je mo¾né prohlásit, ¾e v¹echny cíle kladené na tuto práci se podaøilo splnit. Ka¾dému bodu zadání je zde vìnován oddíl, ve kterém je v¹e podrobnì rozebráno. Prvnímu bodu zadání odpovídá celá druhá kapitola, která je tvoøena popisem protokolu XMPP, na kterém je postavena komunikaèní slu¾ba Jabber. Také je zde rozebrána architektura sítì a základní stavební kameny XMPP protokolu, které jsou vyu¾ívány sítí Jabber jako nástroj k zprostøedkování jednotlivých slu¾eb. Druhý bod zadání úzce souvisí s tvorbou robota, který byl vytvoøen za úèelem získávání dat z Jabber komunikace. Zdrojové soubory této aplikace jsou pøilo¾eny na kompaktním disku. Implementaèní èástí se vìnuje kapitola ètvrtá, kde je rozebrána jak celková programová architektura této práce tak její jednotlivé bloky. První èásti kapitoly páté je splnìn tøetí bod zadání. Jsou zde manuálnì prozkoumaná data, která jsou nashromá¾dìná z Jabber komunikace a následnì ulo¾ená v databázi. Z ruèního prohlí¾ení dat byly vyvozeny dùsledky, které tvoøily základ pro splnìní bodu ètvrtého. Tomuto a následnému bodu zadání je vìnována zbývající èást páté kapitoly, která je doplnìna o pøílohy nacházející se na konci tohoto dokumentu. Pro automatické zkoumání dat byl vyu¾it nástroj RapidMiner, který je spoleènì s dal¹ími vybranými programy popsán na konci kapitoly tøetí. Obsah kapitoly tøetí je zamìøen na popis data miningu a na jeho zaèlenìní do celkového procesu získávání znalostí z databází. Poslednímu bodu zadání je urèena èást v kapitole ètvrté, která se zamý¹lí nad pøípadným pokraèováním této práce.

Jak je uvedeno vý¹e ve¹keré body zadání byly splnìny. Jako roz¹íøení nad rámec zadání bylo zpøístupnìní mo¾nosti vyu¾ít jak robota tak i data ulo¾ená v databázi pro prezentování vlastního statusu na webových stránkách. Pøíklad tohoto pou¾ití je pøilo¾ený na kompaktním disku nebo je k dispozici k nahlédnutí na internetové stránce \url{http://pcpanel-l205.fit.vutbr.cz/jabinfo/}.

Pro nedostateèný poèet u¾ivatelù, kteøí doposud vyu¾ívali tohoto robota, a zároveò by mìli správnì nastavené \uv{rozesílání} roz¹íøených statusù vedlo k následujícímu rozhodnutí. Vìt¹ího dùrazu bylo kladeno na vývoj samotného robota a sbírání i ukládání dat do databáze, ne¾ na následný proces dolování z dat. Za období ètyø mìsícù bylo celkovì nasbíráno kolem 200\,MB dat a pøes 100 tisíc záznamù.