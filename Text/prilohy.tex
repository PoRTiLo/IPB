...
./src/datamining       - zdrojové soubory pro data mining
...
\item [TSQL] (Transact-SQL) --- jazyk pro temporální databáze.
...
\end{figure}
\newpage
Server obdr¾í informace od klienta \textit{user} prostøednictvím zprávy, její¾ obsah je naplnìn daty spojenými s~pøehrávající hudbou. Pomocí elementu \textit{message} ji pøepo¹le v¹em u¾ivatelùm ze seznamu kontaktù u¾ivatele \textit{user}, kteøí se pro odbìr tìchto typù zpráv zaregistrovali. Struktura této zprávy je prezentována na pøíkladu \ref{picSPep2}.
\begin{figure}[h]
...
\end{figure}
\newpage
Zpráva o~pøehrávané hudbì je také pøeposlána v¹em otevøeným spojením u¾ivatele \textit{user}, ukázáno na pøíkladu \ref{picSPep3}.
\begin{figure}[h]
...
Pøestane-li u¾ivatel \textit{user} poslouchat/vysílat informace o~pøehrávané hudbì, provede to pomocí zprávy ukázané na pøíkladu \ref{picSPep4}. Zpráva typu \textit{iq}, ve které je polo¾ka \textit{tune}, nesoucí informace o~skladbì, je prázdná.
%\begin{figure}[h]
...
Tabulka \ref{ETabProcent}, která se nachází ní¾e, plnì zobrazuje procentuální shodu mezi jednotlivými u¾ivateli. Èísla v~prvním øádku a~sloupci odpovídají pøiøazení prezentovanému v~tabulce \ref{ETabPrirazeni}. Barevnì odli¹ené buòky tabulky vyzdvihují vìt¹í procentuální shodu ne¾ jaká je u~ostatních hodnot. Barvì zelené odpovídá interval od 80\,\%\,--\,89\,\% a~pro barvu èervenou 90\,\%\,--\,99\,\%.
\begin{center}
   \begin{landscape}
        \begin{table}[htb]
\subtable{
