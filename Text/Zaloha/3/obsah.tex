...
Pøedmìtem této bakaláøské práce bylo seznámení se s problematikou komunikace pøes Jabber sí», která zde byla rozebrána.
Konkrétním cílem bylo vytvoøení jednoduchého Jabber klienta, který by byl schopen získávat statistická data. Nashromá¾dìná data slou¾ila pro pozdìj¹í analýzu a grafickou reprezentaci informací z nich získaných.
...
Dobøe navr¾ená internetová technologie je tvoøena správnì fungujícími komponenty, které mezi sebou doká¾í vytvoøit spojení a následnì zapoèít komunikaci. Pro popis Jabber architektury v této práci bylo èerpáno z \cite{Adams,Saint}. 
%Dobøe navr¾ená architektura tvoøí základ pro správnì fungující internetovou technologii. Pro její popis v této práci bylo èerpáno z \cite{Adams,Saint}. 
...
Geografická poloha je pøená¹ena ve formì souøadnic popisující pøímo zemìpisnou ¹íøku a délku. Souèasnì lze informaci o poloze pøená¹et i slovnì ve formì adresy. Pøíkladem slovního popisu je ulice, èíslo popisné, mìsto a dal¹í.
%Geografická poloha \cite{Xep0080} je pøená¹ena jak ve formì GPS\footnote{Global Positioning System} souøadnic tak i ve tvaru \uv{civilním}, jako je stát, ulice, mìsto nebo èíslo poschodí v budovì a dal¹í. 
...
%Vý¹e popsáná roz¹íøení nejsou z velké míry podporována aplikacemi, co¾ zpùsobuje nespokojenost mnoha u¾ivatelù vyu¾ívající hlavnì mobilní zaøízení.
...
%Do této doby data pro¹la ji¾ nìkolika kroky. 
Ne¾ se data dostala do tohoto stavu, bylo na nich provedeno nìkolik krokù, ve kterých byla upravována.
V první fázi byla sbírána Jabber komunikace, která je popsána v první kapitole. Druhá fáze byla zamìøena na zú¾ení výsledné mno¾iny a proto byla vybrána jen relevantní data. I pøes tyto kroky relaèní databáze obsahuje velké mno¾ství dat, která se nenachází ve stavu, aby mohla být pou¾ita jako zdroj pro data minig. Jak bude popsáno v následující podkapitole vìt¹ina metod pro dolování dat pracuje pouze s daty, která obsahují kvantitativní promìnné. Za tímto úèelem je potøeba v¹echny atributy tabulky z databáze, které mají být nadále pou¾ívány, pøevést na mìøitelné hodnoty.  
...
%\begin{center}
%\input{bijekce}
%\caption{Bijektivní funkce transformuje hodnoty z $S$ do $T$.}
% \label{bijekce}
%\end{center}
%\end{figure}
%Konkrétní grafická prezentace transformace je ukázána na obrázku \ref{bijekce}, kde definièní obor je tvoøen mno¾inou $S$ a obor hodnot mno¾inou $T$. Jednotlivé prvky v $S$ jsou vyjádøeny svou textovou hodnotou a prvky výsledné mno¾iny $T$ ji¾ hodnotou èíselnou.

Pøi výbìru velikosti hodnoty, pro výslednou mno¾inu $T$, bylo èerpáno z programu Gajim, který je multiplatformní klient s velkou podporou standardù a roz¹iøujících protokolù. 
%Bylo èerpáno právì z jeho funkce, která podle èasových intervalù neèinnosti u¾ivatele, automaticky nastavuje status a hlavnì èíslo priority. Jeliko¾ rozsah mezi hodnotami priorit v této aplikaci mi pøi¹el jako nedostateèný, má zvolená vý¹e priorit pouze dodr¾uje poøadí jednotlivých velikostí.
Hodnoty v mno¾inì $T$ byly zvoleny tak, aby mìly sestupné uspoøádání, a aby bylo mo¾né je dobøe mezi sebou porovnávat. Jsou--li vybrány dvì hodnoty napøíklad available a unavailable, vzdálenost mezi nimi musí být vìt¹í ne¾ vzdálenost napøíklad u hodnot chat a away. 
Na druhou stranu prvky ze vstupní mno¾iny $S$ jsou striktnì definovány podle Jabber standardu, který je popsán v RFC \cite{Rfc3920}.
...
%Napøíklad den 2011--08--23 12:02:12, je  pøevedeno na otevøený interval $\iota$ \textit{\textless 2011--08--23 00:00:00, 2011--08--23 23:59:59\textgreater}.
...
%  \item Hranièní hodnoty intervalu $\iota$ jsou pøevedeny na sekundy a vypoèítán rozdíl mezi nimi.
...
%\textbf{Dal bych do tehle kapitoly popsal tu transformaci temporalnich dat z ridkych na huste  taky by bylo vhodne napsat jakym zpusobem je vyresene vicenasobne prihlaseni. (max(), avg(), min(), sum(), +konstanta)}
...
Pou¾itelnost bude vysvìtlena právì na pøíkladu analýzy nákupního ko¹íku. Podstata pøíkladu je tvoøena zákazníkem a jeho systémem nakupování. Jsou zji¹»ovány produkty, které jsou nakupovány souèasnì. Hledají se nebo--li jsou vytváøeny spoleèné vazby (asociaèní pravidla) mezi výrobky a urèuje se jejich spolehlivost. Na základì tìchto závislostí je upravováno umístìní jednotlivých výrobkù. Obecnì jsou tedy asociaèní pravidla pova¾ována za konstrukci, která z hodnot jedné transakce odvozuje mo¾nost výskytu závislostí v jiných transakcích. Jsou tedy hledány v¹echny vnitøní závislosti existující mezi daty.

%Podle knihy Berky \cite{Berka} je základní my¹lenka asociaèních pravidel \textit{IF--THEN} pøevedena do jiné terminologie:
%\begin{figure}[h]
%\begin{verbatim}
%                             Ant => Suc
%\end{verbatim}
...
%Pøíklad postupu algortimu DBSCAN zobrazuje obrázek \ref{dbscan}. Kde $\varepsilon$--sousedství tvoøí kruh a $MinPts$ má hondotu 3.
%\begin{figure}[h]
%\begin{center}
%\input{dbscan}
...
Hlavním zástupcem u uvedených metod je algoritmus \textit{k}--means, který je popsán ní¾e. Tvoøí základ pro vìt¹inu metod shlukování nejen pro metody rozkladu. K dal¹ím metodám se øadí \textit{k}--medoidù, \textit{k}--modù, \textit{k}--histogramù, fuzzy shluková analýza a dal¹í.
 ...
 %v ka¾dém kroku se pracuje se v¹emi shluky rozdìlení prvkù   se postupnì vylep¹uje.

\subsubsection*{\textit{k}--means}\label{kMeans}
Shlukování pomocí algoritmu \textit{k}--means je pou¾íváno pro data obsahující kvantitativní promìnné a pro data, která nejsou pøíli¹ za¹umìlá. Základní proces je tvoøen iterativním rozdìlováním objektù do tøíd na základì vzdáleností od jejich støedù. Støed nebo--li centroid shluku je vektor, jeho¾ vzdálenost od souètu vzdáleností objektù v této tøídì je minimální. Celý tento proces je prezentován na obrázku \ref{automat} pomocí jednoduchého schématu koneèného automatu. Jednotlivé kroky koneèného automatu odpovídají krokùm \textit{k}--means zobrazeného pomocí algoritmu \ref{algoritmus}.
\begin{figure}[h]
 \begin{center}
...
Obsahem ètvrté kapitoly je popis praktické èásti této práce. Jsou zde charakterizovány jednotlivé prvky, které byly pou¾ity jak pro získání dat, tak pro jejich následné ulo¾ení. 

Cílem této práce je dolování dat z Jabberu. Jak ji¾ bylo døíve napsáno, Jabber je real--time komunikaèní slu¾ba díky ní¾ mohou její u¾ivatelé komunikovat, informovat nebo sdílet svùj status s jinými u¾ivateli. Celá tato vzájemná komunikace skrývá nepøeberné mno¾ství informací o klientech dané sítì. V¹echna tato navzájem vymìnìná nebo poskytnutá data následnì poslou¾í jako zdroj samotnému dolování. Pro jejich uskladnìní je vyu¾ita databáze, jejich¾ strukturální návrh prezentuje obrázek \ref{picDatabase}. 

Druhá èást této kapitoly je zamìøena na Jabber klienta nebo--li robota, který je implementován v jazyce C++ pouze s konzolovým rozhraním. Robot v této práci hraje roli pasivního u¾ivatele, který informace pouze pøijímá. Ve vybraných pøípadech doká¾e u¾ivatele ze svého seznamu kontaktù vyzvat k zaslání odpovìdi s informacemi na odpovìï, o kterou ¾ádal. Struktura samotného robota je popsána ní¾e a reprezentována obrázkem \ref{picRobot}. Na konci èásti o Jabber robotovi je zmínìno o knihovnì \textit{gloox}, kterou je zprostøedkovány v¹echny nále¾itosti Jabber komunikace.
...
Data, která jsou sbírána robotem, jsou ukládána do objektovì--relaèní databáze PostgreSQL\footnote{vyvinul se z projektu Ingres}. PostgreSQL nebo--li také Postgres byl pou¾it ve verzi 8.4.7 a je provozován na operaèním systému Ubuntu Maverick verze 10.10.
\subsection*{Návrh databáze}
Struktura databáze, do které je ukládána ve¹kerá komunikace Jabber robota, vyu¾ívá relaèní model. Obrázkem \ref{picDatabase} jsou prezentovány nejdùle¾itìj¹í èásti databáze. Celou strukturu návrhu je mo¾né nalézt v pøíloze \ref{EDatabase}. V jednotlivých èástech návrhu databáze je poèítáno s jejím druhotným vyu¾itím obsahu, které bude popsáno v dal¹ích oddílech této práce.

%temporalni DB
V dobì návrhu databáze zcela nebylo zøejmé, která data budou následnì analyzována. Z toho dùvodu se struktura databáze sna¾í zachytit v¹echna \uv{dùle¾itá} data. Pøedem nebylo urèen typ znalostí, kterých by se pøi dolování mìlo dosáhnout. Za úèelem uchování dat, je v návrhu databáze obsa¾ena tabulka \textit{xml}, která je nositelem obsahu jak v¹ech pøijatých, tak i odeslaných zpráv. Tabulky \textit{debug}, \textit{level} a \textit{logarea}, které v zú¾eném návrhu databáze, uvedeném na obrázku \ref{picDatabase}, nejsou zobrazeny, jsou urèeny pouze jako doplòkové informace k typu zprávy. Pøíkladem mù¾e být øádek v tabulce xml a sloupcem \textit{area} s hodnotou 32, kterým jsou oznaèeny zprávy spojené s pøekladem IP adres a doménových jmen. Hodnota \textit{level} je pro vìt¹inu zpráv nastavena na hodnotu nula, co¾ znamená ¾e nejde o zprávu obsahující chybu ani varování. Z pohledu návrhu vytváøí tabulka \textit{xml} redundantní data, dalo by se tedy øíct, ¾e je nadbyteèná. Je mo¾né ji nahradit jednotlivými dal¹ími tabulkami, které jsou zamìøeny na konkrétní data. Z druhého pohledu je ale patrné, ¾e tabulka \textit{xml} v atributu \textit{message} obsahuje v¾dy ucelenou jednu zprávu i s elementy, kterými je obalen samotný obsah. Pøíkladem entity, která ji¾ obsahuje konkrétní data bez xml prvkù, je tabulka \textit{message}, která je takté¾ vyobrazena v pøíloze \ref{Estanza}. Jejím obsahem jsou zprávy vzniklé pøi Jabber komunikaci mezi u¾ivatelem a robotem, jeho¾ schopnosti budou popsány ní¾e.

Tabulka \textit{presence} z pohledu XMPP standardu prezentuje jeden ze tøí základních èástí stanzy, element \textit{presence}. Základním cílem této tabulky je shroma¾ïování u¾ivatelských statusù. Tento po¾adavek je splnìn atributem \textit{presence}. Jak ji¾ bylo zmínìno ve tøetí kapitole, v èásti která se zabývá transformací, atribut presence obsahuje hodnoty typu text. Pøíklad v¹ech mo¾ných hodnot, kterých tento atribut mù¾e nabývat je prezentován v pøíloze \ref{Estanza} v èásti zabývající se elementem presence. K dal¹ím atributùm této tabulky patøí \textit{message}, který je reprezentován textovým øetìzcem a roz¹iøuje informace o stavu u¾ivatele. V této èásti je èasto obsa¾en text, který je do statusu pøidán automaticky IM klientem. Jako pøíklad lze uvést vìtu \uv{I'm not here right now} generována aplikací Miranda po del¹ím èasovém úseku neèinnosti. Transformovaný obsah této entity tvoøí dùle¾itou èást pro dal¹í krok v KDD a to v podobì dat, ze kterých budou \uv{dolovány} informace.

Vìt¹ina roz¹íøení standardu XMPP, která jsou popsána ve druhé kapitole, jsou prezentována pomocí pìti tabulek. Konkrétnì to jsou tabulky \textit{geoloc}, \textit{tune}, \textit{mood}, \textit{activity} a \textit{vcard}. Poslední roz¹íøení informující o verzi a jménu IM klienta a systému bude rozebráno v následujícím odstavci v souvislosti s dal¹í tabulkou \textit{resource}. Pro obsah a názvy atributù v¹ech pìti tabulek s roz¹íøeními se staly vzory dokumenty jednotlivých XEP, které je definují. Podrobnìj¹í popis v¹ech atributù lze tedy nalézt právì v tìchto standardech. Tabulky kromì tìchto atributù obsahují také polo¾ku \textit{jidbare}, která, jak ji¾ název naznaèuje, obsahuje pouze èisté JabberID bez resources. Tato skuteènost vyplývá ji¾ ze samotného návrhu XEP protokolù. Adresa u¾ivatele, který posílá serveru zprávu s jedním z mo¾ných roz¹íøení, je také pouze v základním stavu. V tabulce \textit{vcard} jsou také polo¾ky, jejich¾ obsah je tvoøen fotografiemi. Tyto atributy ale nejsou binárního typu, tak jako obrázky samotné, nýbr¾ textového. Obrázky jsou zde ulo¾eny ve stavu, tak jak jsou pøená¹eny po síti, tedy pomocí datového formátu Base64. Base64 je algoritmus, který pøevádí binární data na øetìzec, který je tvoøen pouze znaky ASCII tabulky.

S posledním roz¹íøením nazvaném \textit{Software version} je spjata tabulka resource. Její nìkolik sloupcù tvoøí právì informace o IM klientovi a operaèním systému, na kterém bì¾í. Tyto základní údaje jsou definovány v protokolu \cite{Xep0092}, kde je také mo¾né èerpat bli¾¹í informace. Pokroèilej¹í atributy, jako je \textit{type} a \textit{osversion} nacházejí svùj podklad v XEP dokumentu \cite{Xep0115} takté¾ jako atribut \textit{ver}, který je podrobnìji popsán ve druhé kapitole této práce. V neposlední øadì se zde nachází zmínka o podpoøe audia a videa a» u¾ v podobì \textit{jingle} roz¹íøené nebo \textit{google}. tyto sloupce jsou typu boolean, tudí¾ hodnota \textit{true} znamená podporu daného sloupce. Pøesto tyto vý¹e uvedené údaje nejsou hlavním dùvodem existence této tabulky. Její primární cíl je uchovávat informace o pøipojených u¾ivatelích a v¹ech jejich resources.

Entita resources tvoøí základ pro tabulku \textit{status}. Její obsah je automaticky generován z obsahu tabulky popsané vý¹e. Poèet øádkù odpovídá poètu u¾ivatelù, kteøí jsou v seznamu kontaktù tohoto robota. Primárním cílem je informování o aktuálním stavu u¾ivatele. Je tedy poskytován stav, ve kterém se u¾ivatel nachází s pøihlédnutím na prioritu a resources. Øádek entity obsahuje status a dostupnost u¾ivatele a jeho resources s nejvìt¹í pøihlá¹enou prioritou. Nastane--li situace, ¾e má u¾ivatel více pøipojení pod jedním úètem je vzato v potaz pøihlá¹ení s nejnovìj¹í zmìnou presence.
\begin{figure}[ht]
\begin{center}
\input{DatabaseSmall}
...
\section{Robot}
%Pøedchozí podkapitola se zabývala vhodným ulo¾ením a uskladnìním dat, která jsou sbírána z Jabber komunikace. 
V této èásti bude popsána aplikace, která Jabber komunikaci s ostatními entitami real-time sítì zaji¹»uje. Tento konzolový robot vyvíjený pro operaèní systém Linux také vhodným zpùsobem pracuje s databází, kam jsou jednotlivá získaná data ukládána. Jabber, jádro sí»ové komunikace \cite{Rfc3920}, je implementováno za pomocí volnì dostupné knihovny \textit{gloox}. V porovnáním s jinými knihovnami disponuje lep¹í podporou a dokumentací. Gloox plnì podporuje standart XMPP Core \cite{Rfc3920} a z vìt¹í èásti i standard XMPP IM \cite{Rfc3921}. Dodateènì je plnì podporováno kolem tøiceti XEP standardù napøíklad vcard--temp \cite{Xep0054} a dal¹í. 

Pøi implementaci robota byly vyu¾ity základní prvky objektovì--orientovaného programovaní, jako jsou napøíklad tøídy a dìdiènost. Ka¾dá tøída zde zastupuje jednoznaènì oddìlitelné elementy robota. A» u¾ se jedná o blok komunikující s databází, rùzná roz¹íøení nebo o jádro robota. V¹echny zde jmenované objekty hrají dùle¾itou roli jak v èásti dekompozice na men¹í ucelené celky tak i v celkové komplexnosti programu. Jako pøíklad lze uvést tøídy, které reprezentují jednotlivá pro tuto práci vhodná roz¹íøení, jejich¾ základ je v podobì XEP dokumentù.

\subsection*{Návrh bota}
Struktura návrhu tøíd robota, která je v základní èásti prezentována obrázkem \ref{picRobot}, je velmi podobná struktuøe návrhu databáze, obrázek \ref{picDatabase}. Mnoho atributù z robota a z databáze jsou ve vztahu jedna ku jedné. Samotná komunikace a øízení mezi tìmito entitami je zprostøedkováno pomocí knihovny \textit{libpq}. Libpq je programátorské rozhraní pro databázi PostgreSQL. Je to sada knihovních funkcí v jazyce C, které umo¾òují klientským programùm zadávat dotazy na server a následnì na nì obdr¾et odpovìdi. Pomocí této knihovny je øízen pøenos dat mezi robotem, který sbírá data, a databází, kam jsou ukládána. Navázání spojení a následné jednotlivé dotazy probíhají v blokujícím re¾imu. Údaje potøebné k navázání spojení mezi robotem a databází jsou ulo¾eny v souboru \textit{connect.h} v podobì pøeddefinovaných konstant napøíklad \textit{DB\_USER}. V tomto dokumentu jsou uvedeny v¹echny údaje nutné k navázaní spojení a» u¾ s databází nebo i s komunikaèním Jabber serverem. Tudí¾ se stává hlavní èástí urèenou k editaci pøipojení a Jabber úètu na nìm¾ robot bì¾í. K dal¹ímu pouze textovému dokumentu patøí \textit{const.h},  který je zamìøen k definování jednotlivých pøíkazù a dotazù pro databázi. Jsou to pøíkazy pro tvorbu a aktualizaci tabulek, které jsou takté¾ definované jako konstanty. Identifikaèní názvy, pro snadné odli¹ení, jsou psány velkými písmeny a zaèínají textem \textit{DB\_}.

Tøída, která vyu¾ívá zde doposud popsané konstanty a rozhraní libpq, je nazvaná \textit{Database}. Obsahuje mnoho funkcí, které jsou pøetì¾ovány, a které zaji¹»ují samotnou provázanost mezi daty a databází. Pomocí tìchto funkcí jsou data získaná z komunikace pøená¹ena a ukládána do jednotlivých tabulek databáze. Pro zahájení samotné práce s databázi slou¾í funkce \textit{start()}, která zajistí pøipojení k databázi a provede nutné inicializaèní kroky. Jako je tvorba nových tabulek, pøi prvním zapnutí aplikace, nebo kontrola existence tìchto entit.

Pro roz¹íøení podle dokumentù XEP popsaných v kapitole druhé, v èásti zabývající se relevantními roz¹íøeními, je základ nadtøída Extension. Tato tøída obsahuje základní funkce a parametry pro v¹echna roz¹íøení. Jako pøíklad lze uvést u¾ivatelské jméno klienta, který dané roz¹íøení \uv{¹íøí} po síti. Obsahuje také dvì èistì virtuální metody, slou¾ící k rozparsování èásti xml zprávy a k vyèistìní promìnných. Tøídy, které z Extension dìdí a jsou prezentovány na návrhu robota uvedeném na obrázku \ref{picRobot}, jsou následující: \textit{Geoloc}, \textit{Tune}, \textit{Mood}, \textit{Activity} a \textit{SwVersion}. Samotnou strukturou tìchto roz¹íøení, jak ji¾ bylo uvedeno vý¹e, jsou pomocí parametrù kopírovány vlastnosti jednotlivých XEP dokumentù. Tedy ke ka¾dému atributu xml zprávy existuje jak promìnná uchovávající její obsah tak i tzv. \uv{get} a \uv{set} metoda. Pomocí virtuální metody jsou jednotlivé èásti roz¹íøení rozparsovány, ulo¾eny do tøíd a následnì vlo¾eny do databází. Poslední doposud nezmiòované roz¹íøení v podobì \textit{vcard}, je vyu¾ito z knihovny gloox, která jej implementuje.
...
\subsection*{Roz¹íøení}
==zpravy, odpovida reaguje na to a na to


==pouziti castecneho komentovani podle stylu doxygenu, makefile
%\subsection*{gloox}
%Gloox je stabilní Jabber/XMPP knihovna vydávána pod licencí GNU GPL. Je urèena pro vývoj klienta a komponent. Jeliko¾ je psána v ANSCI C++ je mo¾né ji oznaèit jako multiplatformní\footnote{Linux, Windows, Mac OS X, Symbian/Nokia S60, FreeBSD}.
...
\chapter{Závìr}
POKACOVANI
=tridit lidi do podskupin v kotakt listu, napriklad podle zpravy zaslene s dotazem k autorizaci=: kazda skupina by mohla mit jin prava, jine dotazy an sluzby...mozne rozsirit o ruzne sluzby, ktere poskytuji server, podle dotazu....
%=========================================================================
